    ...
    const sheets = [
      {
        ...
        data: [
          ...
          ['Date Ceased', formatDate(crew.date_ceased)],
        ...
        {loading ? (
          ...
          <Tabs defaultValue="details" className="w-full">
            <TabsList>
              ...
              <TabsTrigger value="trips">Trip Allocations ({trips.length})</TabsTrigger>
              <TabsTrigger value="shifts">Crew Shifts ({shifts.length})</TabsTrigger>
              <TabsTrigger value="drills">Drills ({drillRecords.length})</TabsTrigger>
              <TabsTrigger value="training">Training ({trainingRecords.length})</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="trips" className="space-y-4">
              <div className="flex justify-between items-center">
                <div className="text-sm text-gray-600">Trips allocated to {crew.staff_name}</div>
                {trips.length > 0 && (
                  <Button variant="outline" size="sm" onClick={() => {
                    exportMultiSheetExcel([{
                      name: 'Trip Allocations',
                      headers: ['Trip Name', 'Vessel', 'Start Date', 'End Date', 'Position', 'Status'],
                      data: trips.map(t => [safeValue(t.trip_name), safeValue(t.vessel_name), formatDate(t.start_date), formatDate(t.end_date), safeValue(t.position), safeValue(t.status)]),
                      columnWidths: [25, 20, 15, 15, 15, 12]
                    }], `crew_trips_${crew.staff_name}`);
              ...
              {trips.length > 0 ? (
                ...
                <p className="text-gray-500 text-center py-8">No trip allocations for this crew member.</p>
              )}
            </TabsContent>
            ...
            <TabsContent value="shifts" className="space-y-4">
              <div className="flex justify-between items-center">
                <div className="text-sm text-gray-600">Crew shift logs for {crew.staff_name}</div>
                {shifts.length > 0 && (
                  <Button variant="outline" size="sm" onClick={() => {
                    exportMultiSheetExcel([{
                      name: 'Crew Shifts',
                      headers: ['Shift Start', 'Shift End', 'Vessel', 'Task Performed', 'Total Hours'],
                      data: shifts.map(s => [formatDate(s.shift_start_datetime, true), formatDate(s.shift_stop_datetime, true), safeValue(s.vessel_name), safeValue(s.task_performed), s.total_hours ? `${s.total_hours}h` : '-']),
                      columnWidths: [20, 20, 20, 30, 12]
                    }], `crew_shifts_${crew.staff_name}`);
                  }} className="bg-green-50 hover:bg-green-100 text-green-700 border-green-200">
                    <FileSpreadsheet className="h-4 w-4 mr-2" />Export to Excel
                  </Button>
                )}
            ...
            <TabsContent value="drills" className="space-y-4">
              <div className="flex justify-between items-center mb-3">
                <h3 className="font-semibold text-lg flex items-center gap-2">
                  <Activity className="h-5 w-5 text-blue-600" />Emergency Drills
                </h3>
                {drillRecords.length > 0 && (
                  <Button variant="outline" size="sm" onClick={() => {
                    exportMultiSheetExcel([{
                      name: 'Drills',
                      headers: ['Drill Type', 'Record Date', 'Status', 'Authorized By', 'Notes'],
                      data: drillRecords.map(d => [safeValue(d.drill_type), formatDate(d.record_date, true), safeValue(d.status), safeValue(d.authorized_by), safeValue(d.notes)]),
                      columnWidths: [20, 20, 10, 20, 30]
                    }], `crew_drills_${crew.staff_name}`);
                  }} className="bg-green-50 hover:bg-green-100 text-green-700 border-green-200">
                    <FileSpreadsheet className="h-4 w-4 mr-2" />Export to Excel
                  </Button>
                )}
              ...
              {drillRecords.length > 0 ? (
                ...
                <p className="text-gray-500 text-center py-8">No drill records for this crew member.</p>
              )}
            </TabsContent>
            ...
            <TabsContent value="training" className="space-y-4">
              <div className="flex justify-between items-center mb-3">
                <h3 className="font-semibold text-lg flex items-center gap-2">
                  <FileText className="h-5 w-5 text-orange-600" />Procedure Training
                </h3>
                {trainingRecords.length > 0 && (
                  <Button variant="outline" size="sm" onClick={() => {
                    exportMultiSheetExcel([{
                      name: 'Training',
                      headers: ['Procedure', 'Emergency Type', 'Training Date', 'Status', 'Authorized By', 'Notes'],
                      data: trainingRecords.map(t => [safeValue(t.procedure_title), safeValue(t.emergency_type), formatDate(t.training_date, true), safeValue(t.status), safeValue(t.authorized_by), safeValue(t.notes)]),
                      columnWidths: [25, 15, 20, 10, 20, 30]
                    }], `crew_training_${crew.staff_name}`);
                  }} className="bg-green-50 hover:bg-green-100 text-green-700 border-green-200">
                    <FileSpreadsheet className="h-4 w-4 mr-2" />Export to Excel
                  </Button>
                )}
