    ...
    const fetchVesselData = async () => {
      if (vessel && vessel.id && mode === 'edit') {
        ...
        try {
          ...
          // Filter contacts by vessel (show if vessel_id matches OR if no vessel_id is set - global contacts)
          const vesselContacts = (contactsRes.data || []).filter(c => 
            c.vessel_id === vessel.id || c.vessel_ids?.includes(vessel.id) || (!c.vessel_id && !c.vessel_ids)
          );
          // Filter procedures by vessel (show if vessel in vessel_ids OR no vessel_ids set - global procedures)
          const vesselProcedures = (proceduresRes.data || []).filter(p => 
            p.vessel_ids?.includes(vessel.id) || p.vessel_id === vessel.id || (!p.vessel_ids || p.vessel_ids.length === 0)
          );
          setVesselEmergency({
                      ...
                      <div className="border rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <Button 
                            variant="outline" 
                            size="sm"
                            onClick={() => window.location.href = `/emergency?tab=drills&vessel_name=${encodeURIComponent(vessel?.vessel_name || '')}`}
                          >
                            <ExternalLink className="h-4 w-4 mr-2" />
                            Manage in Emergency
                          </Button>
