      ...
      <div data-testid="incident-reporting" className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button data-testid="report-incident-button" className="bg-red-500 hover:bg-red-600">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select data-testid="severity-select" value={formData.severity} onChange={(e) => setFormData({ ...formData, severity: e.target.value })} className="w-full p-2 rounded-md bg-gray-50 border-gray-300 text-white border">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input data-testid="incident-location-input" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} required className="bg-gray-50 border-gray-300" />
                  </div>
                </div>
                <div>
                  <Label>Description</Label>
                  <Textarea data-testid="incident-description-input" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} required className="bg-gray-50 border-gray-300" rows={4} />
                </div>
                <div>
                  ...
                  {formData.persons_involved.map((person, idx) => (
                    ...
                    }} className="bg-gray-50 border-gray-300 mb-2" placeholder="Name..." />
                  ))}
                ...
                <div>
                  ...
                  <Textarea data-testid="immediate-action-input" value={formData.immediate_action} onChange={(e) => setFormData({ ...formData, immediate_action: e.target.value })} required className="bg-gray-50 border-gray-300" rows={3} />
                </div>
                <Button type="submit" data-testid="submit-incident-button" className="w-full bg-red-500 hover:bg-red-600">Submit Report</Button>
              </form>
            </DialogContent>
          </Dialog>
        </div>
        ...
        {vessels.length > 1 && (
          <div className="flex items-center gap-4">
            ...
            <select data-testid="vessel-select-incidents" value={selectedVessel?.id || ''} onChange={(e) => setSelectedVessel(vessels.find(v => v.id === e.target.value))} className="px-4 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-900">
        ...
        {incidents.length === 0 ? (
          <Card className="bg-white border-gray-200">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-gray-600">Good safety record!</p>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-4">
            {incidents.map((incident) => (
              <Card key={incident.id} data-testid={`incident-card-${incident.id}`} className="bg-white border-gray-200">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="text-lg font-semibold text-white capitalize">{incident.incident_type.replace('_', ' ')}</h3>
                      <p className="text-sm text-gray-600">{incident.location} • {new Date(incident.date).toLocaleString()}</p>
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="text-sm text-gray-600 mb-1">Immediate Action:</p>
                      <p className="text-gray-900">{incident.immediate_action}</p>
                    </div>
