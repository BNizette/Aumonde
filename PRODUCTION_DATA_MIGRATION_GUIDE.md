...
### Steps:
1. Go to preview: https://vesselsync-fix.preview.emergentagent.com
...
fetch('https://vesselsync-fix.preview.emergentagent.com/api/backup/export', {
...
```bash
# Login and get token
TOKEN=$(curl -s -X POST "https://vesselsync-fix.preview.emergentagent.com/api/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email": "admin@test.com", "password": "Admin123!"}' | jq -r '.access_token')

# Download backup
curl -H "Authorization: Bearer $TOKEN" \
  "https://vesselsync-fix.preview.emergentagent.com/api/backup/export" \
  -o amsa_backup.json
